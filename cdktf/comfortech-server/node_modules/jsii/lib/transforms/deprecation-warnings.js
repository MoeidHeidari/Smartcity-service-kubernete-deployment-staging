...
class DeprecationWarningsInjector {
    ...
    process(assembly, projectInfo) {
        ...
        for (const type of Object.values(types)) {
            ...
            statements.push(ts.createExpressionStatement(ts.createCall(ts.createPropertyAccess(ts.createIdentifier(VISITED_OBJECTS_SET_NAME), 'add'), undefined, [ts.createIdentifier(PARAMETER_NAME)])));
            const tryStatements = [];
            ...
            if (spec.isEnumType(type) && type.locationInModule?.filename) {
                ...
                for (const member of Object.values(type.members ?? [])) {
                    if (spec.isDeprecated(member)) {
                        ...
                        const condition = ts.createBinary(ts.createIdentifier(PARAMETER_NAME), ts.SyntaxKind.EqualsEqualsEqualsToken, ts.createPropertyAccess(ts.createPropertyAccess(ts.createIdentifier(LOCAL_ENUM_NAMESPACE), type.name), member.name));
            ...
            statements.push(ts.createTry(ts.createBlock(tryStatements), undefined, ts.createBlock([
                ts.createExpressionStatement(ts.createCall(ts.createPropertyAccess(ts.createIdentifier(VISITED_OBJECTS_SET_NAME), 'delete'), undefined, [ts.createIdentifier(PARAMETER_NAME)])),
            ])));
            const paramValue = ts.createParameter(undefined, undefined, undefined, PARAMETER_NAME);
            const functionName = fnName(type.fqn);
            const functionDeclaration = ts.createFunctionDeclaration(undefined, undefined, undefined, functionName, undefined, [paramValue], undefined, createFunctionBlock(isEmpty ? [] : statements));
...
function processInterfaceType(type, types, assembly, projectInfo, statementsByProp = new Map(), excludedProps = new Set()) {
    for (const prop of Object.values(type.properties ?? {})) {
        ...
        if (spec.isDeprecated(prop) || spec.isDeprecated(type)) {
            ...
            const statement = createWarningFunctionCall(fqn, deprecatedDocs, ts.createBinary(ts.createStringLiteral(prop.name), ts.SyntaxKind.InKeyword, ts.createIdentifier(PARAMETER_NAME)), undefined);
...
function createFunctionBlock(statements) {
    if (statements.length > 0) {
        const validation = ts.createIf(ts.createBinary(ts.createIdentifier(PARAMETER_NAME), ts.SyntaxKind.EqualsEqualsToken, ts.createNull()), ts.createReturn());
...
function createWarningFunctionCall(fqn, message = '', condition, includeNamespace = false) {
    ...
    const mainStatement = ts.createExpressionStatement(ts.createCall(ts.createIdentifier(functionName), undefined, [
...
class Transformer {
    ...
    visitor(node) {
        if (ts.isMethodDeclaration(node) && node.body != null) {
            ...
            return ts.updateMethod(node, node.decorators, node.modifiers, node.asteriskToken, node.name, node.questionToken, node.typeParameters, node.parameters, node.type, ts.updateBlock(node.body, [
                ...wrapWithRethrow(statements, ts.createPropertyAccess(ts.createThis(), node.name.getText(node.getSourceFile()))),
            ...
            this.warningCallsWereInjected =
                this.warningCallsWereInjected || statements.length > 0;
            return ts.updateGetAccessor(node, node.decorators, node.modifiers, node.name, node.parameters, node.type, ts.updateBlock(node.body, [
                ...wrapWithRethrow(statements, ts.createPropertyAccess(ts.createCall(ts.createPropertyAccess(ts.createIdentifier(NAMESPACE), GET_PROPERTY_DESCRIPTOR), undefined, [
                    ts.createThis(),
            ...
            this.warningCallsWereInjected =
                this.warningCallsWereInjected || statements.length > 0;
            return ts.updateSetAccessor(node, node.decorators, node.modifiers, node.name, node.parameters, ts.updateBlock(node.body, [
                ...wrapWithRethrow(statements, ts.createPropertyAccess(ts.createCall(ts.createPropertyAccess(ts.createIdentifier(NAMESPACE), GET_PROPERTY_DESCRIPTOR), undefined, [
                    ts.createThis(),
                    ts.createLiteral(node.name.getText(node.getSourceFile())),
                ]), 'set')),
            ...
            this.warningCallsWereInjected =
                this.warningCallsWereInjected || statements.length > 0;
            return ts.updateConstructor(node, node.decorators, node.modifiers, node.parameters, ts.updateBlock(node.body, insertStatements(node.body, wrapWithRethrow(statements, node.parent.name))));
        }
    ...
    getStatementsForDeclaration(node) {
        ...
        if (classSymbolId && this.typeIndex.has(classSymbolId)) {
            ...
            const method = methods.find((method) => method.name === node.name?.getText());
            if (method) {
                return this.getStatements(classType, method);
            }
            const properties = classType?.properties ?? [];
            const property = properties.find((property) => property.name === node.name?.getText());
            if (property) {
    ...
    getStatements(classType, method) {
        ...
        for (const parameter of Object.values(method.parameters ?? {})) {
            ...
            if (parameterType) {
                ...
                statements.push(ts.createExpressionStatement(ts.createCall(ts.createIdentifier(functionName), undefined, [
...
function createTypeHandlerCall(functionName, parameter) {
    return ts.createIf(ts.createPrefix(ts.SyntaxKind.ExclamationToken, ts.createCall(ts.createPropertyAccess(ts.createIdentifier(VISITED_OBJECTS_SET_NAME), ts.createIdentifier('has')), undefined, [ts.createIdentifier(parameter)])), ts.createExpressionStatement(ts.createCall(ts.createIdentifier(functionName), undefined, [
...
function createDuplicateEnumValuesCheck(type) {
    return ts.createIf(ts.createBinary(ts.createPropertyAccess(ts.createCall(ts.createPropertyAccess(ts.createCall(ts.createPropertyAccess(ts.createIdentifier('Object'), 'values'), undefined, [
...
function wrapWithRethrow(statements, caller) {
    ...
    return [
        ts.createTry(ts.createBlock(statements), ts.createCatchClause(ts.createVariableDeclaration('error'), ts.createBlock([
            ...
            ts.createIf(ts.createBinary(ts.createBinary(ts.createPropertyAccess(ts.createPropertyAccess(ts.createIdentifier('process'), 'env'), 'JSII_DEBUG'), ts.SyntaxKind.ExclamationEqualsEqualsToken, ts.createLiteral('1')), ts.SyntaxKind.AmpersandAmpersandToken, ts.createBinary(ts.createPropertyAccess(ts.createIdentifier('error'), 'name'), ts.SyntaxKind.EqualsEqualsEqualsToken, ts.createLiteral(DEPRECATION_ERROR))), ts.createBlock([
                ts.createExpressionStatement(ts.createCall(ts.createPropertyAccess(ts.createIdentifier('Error'), 'captureStackTrace'), undefined, [ts.createIdentifier('error'), caller])),
            ])),
