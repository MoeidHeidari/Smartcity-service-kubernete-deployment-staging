...
export class ClassName implements IInterface {
  ...
  public methodCall(): void {
    ...
    fs.writeJsonSync(path.join(tmpDir, spec_1.SPEC_FILE_NAME), { ...compilationResult.assembly, targets }, {
        spaces: 2,
    });
...
test('will read translations from cache even if they are dirty', async () => {
    ...
    try {
        ...
        const schema = await fs.readJson(path.join(infusedAssembly.moduleDirectory, '.jsii.tabl.json'));
        ...
        const translated = await fs.readJson(path.join(infusedAssembly.moduleDirectory, '.jsii.python'));
...
export class ClassName implements IInterface {
  ...
  public methodCall(): void {
    ...
    fs.writeJsonSync(path.join(tmpDir, spec_1.SPEC_FILE_NAME), { ...compilationResult.assembly, targets: { ...targets, [(0, target_language_1.targetName)(target_language_1.TargetLanguage.GO)]: undefined } }, {
        spaces: 2,
    });
    for (const [file, content] of Object.entries(compilationResult.files)) {
        fs.writeFileSync(path.resolve(tmpDir, file), content, 'utf-8');
    ...
    Object.values(target_language_1.TargetLanguage).forEach((lang) => {
        if (lang === target_language_1.TargetLanguage.GO) {
            expect(fs.pathExistsSync(path.join(outdir, `${spec_1.SPEC_FILE_NAME}.${lang}`))).toBe(false);
        }
        else {
            expect(fs.statSync(path.join(outdir, `${spec_1.SPEC_FILE_NAME}.${lang}`)).isFile()).toBe(true);
        }
